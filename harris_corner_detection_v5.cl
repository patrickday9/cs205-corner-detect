    ...
    if (x < w && y < h)
      {
        ...
        {
          zero += buffer[(buf_y + (psn - halo)) * buf_w + (buf_x - 0)] * local_zero_kernel[psn];
          first += buffer[(buf_y + (psn - halo)) * buf_w + (buf_x - 0)] * local_direct_der[psn];
    ...
    if (x < w && y < h)
      {
        ...
        float Wxy_filter = 0;
         ...
         for (int psn = 0; psn < window_length; psn++) 
        {
          Wxx_filter += buffer_Wxx[(buf_y + (psn - halo)) * buf_w + (buf_x + 0)] * local_zero_kernel[psn];
          Wyy_filter += buffer_Wyy[(buf_y + (psn - halo)) * buf_w + (buf_x + 0)] * local_zero_kernel[psn];
          Wxy_filter += buffer_Wxy[(buf_y + (psn - halo)) * buf_w + (buf_x + 0)] * local_zero_kernel[psn];
        }
