...
def test_match_sort(desc1, desc2, threshold=0.5):
...
def parallel_match_sort(harris_im1, harris_im2, filtered_coords1, filtered_coords2, runs=1, 
                        verbose=True, opencl_file='match_pp_online_sort_v5.cl', run_serial=False):
    ...
    # Setup Output for Parallel Sorting
    sort_matrix = np.zeros_like(py_match1).astype(np.float32)
    ind_matrix = np.zeros_like(py_match1).astype(np.int32)
    gpu_sort = cl.Buffer(context, cl.mem_flags.READ_WRITE, sort_matrix.size * 4)
    gpu_ind = cl.Buffer(context, cl.mem_flags.READ_WRITE, ind_matrix.size * 4)
    ...
    for avg in xrange(1, runs+1):
        ...
        # Reinitialize Global Parameters and Width
        global_size_sort = (py_match1.shape[1], py_match1.shape[0])
        width_sort = np.int32(py_match1.shape[1])

        # Call OpenCL Parallel Sorting Algorthim
        event_sort1 = program.parallel_sort(queue, global_size_sort, local_size, 
                            gpu_match1, gpu_sort, gpu_ind, width_sort)

        # Copy Matrix Indicies Back to CPU
        cl.enqueue_copy(queue, ind_matrix, gpu_ind, is_blocking=False)
    ...
    parallel_match1 = ind_matrix[:,0] 
    
    if run_serial == True:
        # Run the serial version of match an return match matrix
        serial_match1, cpu_d = test_match_sort(d1, d2)
    ...
    # Setup Output for Parallel Sorting
    sort_matrix2 = np.zeros_like(py_match2).astype(np.float32)
    ind_matrix2 = np.zeros_like(py_match2).astype(np.int32)
    gpu_sort2 = cl.Buffer(context, cl.mem_flags.READ_WRITE, sort_matrix2.size * 4)
    gpu_ind2 = cl.Buffer(context, cl.mem_flags.READ_WRITE, ind_matrix.size * 4)
    ...
    for avg in xrange(1, runs+1):
        ...
        # Reinitialize Global Parameters and Width
        global_size_sort2 = (py_match2.shape[1], py_match2.shape[0])
        width_sort2 = np.int32(py_match2.shape[1])

        # Call OpenCL Parallel Sorting Algorthim
        event_sort1 = program.parallel_sort(queue, global_size_sort2, local_size,
                            gpu_match2, gpu_sort2, gpu_ind2, width_sort2)

        # Copy Matrix Indicies Back to CPU
        cl.enqueue_copy(queue, ind_matrix2, gpu_ind2, is_blocking=False)
    ...
    parallel_match2 = ind_matrix2[:,0] 
    
    if run_serial == True:    
        # Run the serial version of match and return match matrix
        serial_match2, cpu_d2 = test_match_sort(d2, d1)
...
if __name__ == '__main__':
    ...
    # Stand Along Version Runs Harris 
    wid = 1
    harris_im1 = compute_harris_response(np_im1, wid) 
    filtered_coords1 = get_harris_points(harris_im1, min_dist=10, threshold=0.1)
    harris_im2 = compute_harris_response(np_im2, wid) 
    filtered_coords2 = get_harris_points(harris_im2, min_dist=10, threshold=0.1)

    # Stand Alone Version Runs Harris
    parallel_matches = parallel_match_sort(harris_im1, harris_im2, filtered_coords1, filtered_coords2, \
        opencl_file='match_pp_online_sort_v5.cl', runs = num_runs, run_serial=True)
    
    # Plot results
    plot_matches(image1, image2, filtered_coords1, filtered_coords2, parallel_matches, 'matching_final.jpg')
